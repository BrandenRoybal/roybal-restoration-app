...
| 2 | Writers stamp `contactId` | roybal-lead, web-agent, phone `createLead` (+ `channel:'phone'`), spine sync, notify inbound, gmail match, publishPortal | M | 1 |
| 3 | `229_contact_links.sql` | link columns + `contact_timeline` view | S | 1 |
| 4 | Board lead lifecycle | blob fields, source badge, follow-up chip + brief nag, won/lost, Lost archive filter, dedicated pipeline view (decision 7) | M | 2 |
| 5 | Admin contacts panel → contact page | panel first; then admin's first hash router | M | 3 |
...
| 8 | CF-2 live job | who's-coming, drying card, documents, SMS bridge (webhook + mirror; Twilio is already live) | M–L | 7 (docs/drying need only 2) |
...
4. ⏸ **TABLED 2026-08-13 — marketing opt-in copy and placement.** Owner researching consent wording and where to collect it. CF-5 does not start until this closes.
5. ⏳ **AWAITING LINK — review destination.** Owner has the destination and will supply it; the ask wiring (CF-4) can build against a config value meanwhile.
