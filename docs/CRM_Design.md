...
| 11 | CF-5 outreach | 🟡 **IN PROGRESS** — campaign budget class ✅ **SHIPPED 2026-08-15**: kind `campaign` in roybal-notify, gated by `campaignGate` (pure, 12 node checks) — send-layer consent (exactly-one live contact + `marketing_opt_in`, never waivable, so an upstream segmentation bug cannot text a non-consenter), its own `SMS_CAMPAIGN_CAP` ceiling (default 100/mo), refused at the shared reserve floor (can never starve owner alerts), quiet-hours guarded by the unknown-kind default, STOP honored by Twilio a layer down. Campaigns panel (segments + human-approved sends) ✅ **SHIPPED 2026-08-16**: admin dashboard panel — opted-in roster as a curated checkbox segment, {name} personalization, sequential sends re-gated per recipient, resume-not-repeat via the campaign tag (client unchecks already-texted + roybal-notify refuses `campaign_duplicate`), shared-number and non-US contacts excluded up front, honest sent/refused/not-attempted arithmetic, composer latched against the 45s dashboard repaint. Adversarially reviewed: 10 findings fixed. Remaining: opt-in collection lands with PR #143; then write + send the first seasonal campaign | M | 1, 10 |
