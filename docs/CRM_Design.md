...
| 11 | CF-5 outreach | 🟡 **IN PROGRESS** — campaign budget class ✅ **SHIPPED 2026-08-15** (PR #142: kind `campaign` via `campaignGate` — send-layer consent, own `SMS_CAMPAIGN_CAP` ceiling default 100/mo, refused at the shared reserve floor, quiet-hours guarded; 12 node checks). Opt-in collection ✅ **SHIPPED 2026-08-15**: portal session-gated "Seasonal tips & reminders" card (verified-phone consent; `prefs`/`setMarketing` gateway actions), site quote-form checkbox (applies ONLY to the contact that submission creates — the public lane never enriches an existing person), admin editor stamps transitions; `marketing_opt_in_at` (migration 240) records when. Remaining: segments, weather/seasonal campaigns, human-approved send flow | M | 1, 10 |
