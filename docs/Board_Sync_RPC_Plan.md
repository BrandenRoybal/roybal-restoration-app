...
- **Tombstones are excluded** by `deleted = false` — this only ever patches a live job. The board's settings blob DOES live in this table since 2026-09 (the reserved fixed uuid `00000000-0000-0000-0000-000000000001` — see `apps/board/js/settingsync.js`; the old `'__settings__'` key could never land in the `uuid` column). It's no patch hazard in practice: the patchers key off job fields (`contactId`, `qbJobcodeId`) the settings blob doesn't carry — but any future caller passing arbitrary ids should treat that row as reserved.
