...
**Roybal Construction LLC · Version 1.3 · Effective 2026-09-06**
...
| **Equipment type** | Freehand. `Air mover`, `Air mover  ` (trailing spaces) and `Axial Air Mover` are three strings for the same class — and the packet's two equipment counters normalise them differently, so the narrative and the sizing table print different numbers from the same rows. | Exactly four strings, copied character for character. Make, model and rating go in Notes. |
...
- **No trailing space.** No space after the last letter. On the equipment table `Attic ` and `Attic` are two different rooms, and the packet counts them separately. (The photo gallery happens to trim spaces before it groups; the equipment table does not. Type it clean everywhere and you never have to remember which is which.)
...
**Deliberately left alone, and why:** the added-time estimates in §5 are estimates, labelled as such — measure them on the reference job (§12.3) rather than replacing one unsourced number with another. Three things v1.3 found and did **not** change, because each needs a decision rather than an edit: §3.1's justification for the no-trailing-space rule (the *rule* is right, but the packet counter it blames trims the string, so the stated consequence is wrong), §2's Before/Now row attributing the two-counters divergence partly to trailing spaces (only `Axial Air Mover` actually diverges them, which §6.2 states correctly), and §12.0's closing sequence, which has the office building the master asset list before the shop day that produces it. The `+25 to +40` correction in §7.2 keeps its worked example (78°F/31% = 44 GPP against 70°F/34% = 37 GPP, GD −7): it was re-checked in 1.3 against the app's own psychrometric function and is exact, and a crew told a normal reading is a mistake needs to see the arithmetic, not just the conclusion. Section structure, the shot list, the cold-weather lane, §6.1–§6.3 and §12.1–§12.3 were checked and left as they were.
