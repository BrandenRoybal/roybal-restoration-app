...
**How to answer.** Every item has a default, and the default is what §1–§8 already assume — silence is consent and the roadmap runs as written. Group A changes the recommendation itself and is wanted before E0. Group B changes the order or size of phases and is wanted before the phase it names. Group C is detail we seed either way. Reply inline under any item, or just mark the ones you want to talk through. Bracketed tags — `[A-0.1]`, `[A-7.12]` — are the register at the end; section references are to the decision record ("Decision record §n") and the numbered parts ("§2.4").
...
| A-0.3 | MCP is propose-only; you set per-division amount thresholds | Agents or tokens approve their own work | §2.4, ADR-03 |
