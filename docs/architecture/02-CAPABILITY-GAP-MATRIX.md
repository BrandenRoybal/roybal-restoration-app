...
3. **Equipment as events (M).** Enum the type, give each unit an asset id and location, derive per-day counts *from* placements; make the per-unit path the fast one (the fill handle at `forms.js:648-682` already exists). Unlocks 2, 1, 4.
...
| G-3 → F-014 | High | Equipment is per-day count strings on psychrometric rows plus a free-text-typed unit table with no asset identity | no | blocks | Unlock #3: equipment events with enum type, asset id, location, placed/removed; derive counts from events. |
...
### G-3 (F-014) — Equipment is per-day count strings on psychrometric rows plus a free-text-typed unit table with no asset identity

**Severity:** High · **Growth ceiling:** no · **Automation impact:** blocks

**Evidence:** apps/field/js/model.js:350-361 (blankEquipRow type free text; blankPsychroRow dehu/am/scrub) · apps/field/js/dryingcalc.js:152-163 (type classified by regex) · apps/field/js/model.js:330 (equipmentPlan icons not linked to rows) · (live) dryingLogs[0]: dehu:"2", am:"20", scrub:"1", no unit rows
