...
1. **Paying twice.** Twelve more months on this shape means twelve months of features built in the blob and in browser executors that all have to be redone when a backbone arrives, while the data the north star needs (line-item justification, per-location readings, submittal records) is discarded on every job. **Correction, 2026-09-07:** equipment placement is the exception and belongs on the other side of this ledger — `dryingLogs[].equipment[]` already holds 116 per-unit rows across 7 jobs, 113 of them complete with asset, type, location, placed, removed and hours (live). The crew is already doing the hard part; what is missing is identity discipline, not the behaviour. See the corrected reading under recommendation 3.
...
3. **Ship unbilled-item detection on mitigation jobs as the first new capability, honestly scoped:** a deterministic, zero-AI reconciliation of per-day equipment counts, logged hours, and monitoring visits against invoice lines, landing as proposals the office approves. It is an internal leak detector, not carrier-grade justification, until equipment *identity* is trustworthy — and it is the first thing that exercises propose → approve → execute → record end to end. **Corrected 2026-09-07:** the original text said "until equipment placement events exist." They already exist and are already habitual (113 of 116 rows complete, 7 jobs, live). The defect is narrower and cheaper to fix than the review first implied: asset numbers collide across fleet classes — asset `101` is logged as an air mover on three jobs and as a `Dehumidifier Dry-Eaze Xi7000` on a fourth — and the type string is freehand and unnormalised (`Air mover`, `Air mover   `, `Axial Air Mover` are three spellings of one thing). `location` is free text with no link to a room record. Enum the type, make asset tags unique per fleet class, and bind `location` to a room id, and per-unit day counts become defensible without asking the crew to do anything new. That moves carrier-grade equipment justification earlier and makes it a smaller change than the roadmap assumed.
...
**The one open question that matters most:** whether you are willing to change the crew's capture shape this year (room identity, per-location readings, and trustworthy equipment identity). **Answered YES by the owner on 2026-09-06**; the training document is [05-CREW-CAPTURE-SOP.md](05-CREW-CAPTURE-SOP.md). The original wording asked for "equipment as placement events" and was wrong to: those events are already captured (see recommendation 3). Everything in capabilities 1 through 3 above "internal leak detector" quality depends on it, it is the only change crews will feel, and it decides whether the first automation stays a v0 or becomes carrier-grade. See [04-OPEN-QUESTIONS.md](04-OPEN-QUESTIONS.md), group A.
