...
function openHelpModal() {
  ...
  const VIEWS = [
    ...
    ["Crew", "A daily magnet board. Drag a guy between job columns to assign him; pick a day with ‹ ›. Drop on “Out today” for a no-show. The Move toggle sets the reach: just this day, this day on (cycle guys through a long job), or the whole job."],
    ["Day", "One day, hour by hour, one row per crew member — drawn from real QuickBooks Time clock-ins, so every colour change in a row is a drive between jobs."],
    ...
    h("div", { class: "help" },
      ...
      sec("Scheduling",
        ...
        h("ul", { class: "help__ul" },
          ...
          li(h("strong", {}, "The plan corrects itself"), " — once QuickBooks Time hours land on a phase-staffed job, its bars follow reality: each phase books only its remaining hours (estimate − logged) from today forward, and a phase marked done books nothing. A job staffed at the job level instead spreads its estimated hours evenly across its whole start→target window."),
          li(h("strong", {}, "On Hold books nobody"), " — a held job keeps its dates on the Gantt and calendar but stops claiming crew: no overloads, no schedule texts, off My Week. Everything comes back the moment the hold lifts."))),

      sec("Crew & assignments",
        h("ul", { class: "help__ul" },
          li("Open ", h("strong", {}, "Roster"), " to add members. The ", h("em", {}, "app login email"), " links a person to their Field-app sign-in (that's what powers My Week and their schedule text), the QuickBooks Time link ties them to their timesheets, and the bio photo/years/certs feed the portal's “Meet your crew” card."),
          li("Assign crew inside the job editor, or per-phase. Each person shows as a colored initials circle on the card."),
          li("Every workday morning, everyone with jobs that day gets a ", h("strong", {}, "schedule text"), " — built from the real dispatch (today's plan, day moves on the Crew view, and logged hours), not leftover estimates. The Roster's “daily schedule text” switch turns it off per person, for crew you'd rather tell yourself (📞 marks them)."),
          li("Crew see their own two weeks in the Field app under ", h("strong", {}, "📅 My Week"), " — the same live schedule as this board, matched by the email they sign in with."),
      ...
      sec("Logging hours & labor",
        h("ul", { class: "help__ul" },
          li("Hours arrive on their own: crew clock-ins sync from ", h("strong", {}, "QuickBooks Time"), " and land on the linked job, and phases match themselves from the timesheet notes."),
          li("Open a job and use ", h("strong", {}, "Time logged"), " for the odd manual entry — who worked, the date, and hours."),
      ...
      sec("Card symbols",
        ...
        h("div", { class: "help__legend" },
          ...
          leg(h("span", { class: "chip chip--sm is-warn" }, "⚠"), "Someone on this job is booked past their shift that day (hover for who and the hours)"),
          leg(h("span", { class: "chip chip--sm is-warn" }, "⚠ No QB link"), "A schedule-truth chip — the schedule can't see this job: no QuickBooks Time link, no hours since start, or a phase at its estimate nobody marked done"),
      ...
      sec("Planning tools",
        h("ul", { class: "help__ul" },
          ...
          li(h("strong", {}, "⚠ Overloads"), " appears in the toolbar when someone's booked past their shift. The hours are the plan's own arithmetic — a job's estimated hours spread across its start→target days, split among its crew — so a big estimate on a tight window with a small crew reads as 11-hour days even when nobody typed an 11 anywhere. Only today and later are graded (last week can't be re-crewed), and every row lists who's free that day. Fix one by adding crew, moving the target date, or correcting the estimate."),
          li(h("strong", {}, "Schedule-truth chips"), " say when the schedule is guessing: ", h("em", {}, "No QB link"), " — no QuickBooks Time jobcode and nothing logged, so hours can't reach the job; ", h("em", {}, "0h since start"), " — the job started but no hours have landed, so the engine still assumes all the work remains; ", h("em", {}, "“…” done?"), " — a phase reached its estimated hours but nobody checked it off, so its leftover keeps sliding the plan right."),
