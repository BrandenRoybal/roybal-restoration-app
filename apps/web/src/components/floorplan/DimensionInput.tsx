...
export default function DimensionInput({ label, currentFeet, onSave, onCancel, minFt = 0.1, maxFt = 200 }: Props) {
  ...
  return (
    ...
    >
      <div className="bg-[#0f1b2d] border border-[#1f3354] rounded-2xl p-6 w-80 shadow-2xl">
        ...
        <input
          ...
          className="w-full bg-[#16263d] border border-[#1f3354] rounded-xl px-4 h-11 text-base text-slate-200 font-mono focus:outline-none focus:border-[#f26a21] transition-colors"
        ...
        <div className="flex gap-3 mt-5">
          <button
            onClick={commit}
            className="flex-1 bg-[#f26a21] hover:bg-[#d4520f] text-[#16263d] font-bold text-sm h-10 rounded-xl transition-colors"
          >
            Apply
          </button>
          <button
            onClick={onCancel}
            className="flex-1 bg-[#1f3354] hover:bg-[#293548] text-slate-300 font-bold text-sm h-10 rounded-xl transition-colors"
          >
            Cancel
          </button>
        </div>
