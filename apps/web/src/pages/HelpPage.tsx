...
const Tip = ({ children }: { children: React.ReactNode }) => (
  <div className="flex gap-2.5 bg-[#f26a21]/10 border border-[#f26a21]/20 rounded-xl p-3 my-3">
    <Info size={15} className="text-[#f26a21] flex-shrink-0 mt-0.5" />
...
const Step = ({ n, children }: { n: number; children: React.ReactNode }) => (
  <div className="flex gap-3 mb-2">
    <span className="w-6 h-6 rounded-full bg-[#f26a21] text-[#16263d] text-xs font-black flex items-center justify-center flex-shrink-0 mt-0.5">{n}</span>
    <p className="text-sm text-slate-300">{children}</p>
  </div>
);
...
const Bullet = ({ children }: { children: React.ReactNode }) => (
  <div className="flex gap-2.5 mb-1.5">
    <CircleCheckBig size={14} className="text-[#f26a21] flex-shrink-0 mt-0.5" />
    <p className="text-sm text-slate-300">{children}</p>
  </div>
);
...
const SECTIONS: Section[] = [
    ...
    content: (
      <div>
        <p className="text-sm text-slate-400 mb-4">
          A <strong className="text-white">Job</strong> is the central record for every loss event — it holds all photos, moisture readings, equipment, scope, and reports in one place.
        </p>

        <H3>Create a new job</H3>
        <Step n={1}>Click <strong className="text-white">New Job</strong> in the Jobs page (top-right button).</Step>
        ...
        <Step n={4}>Click <strong className="text-white">Create Job</strong>. The job number is assigned automatically.</Step>

        <H3>Job statuses</H3>
        <p className="text-sm text-slate-400 mb-3">Every job moves through 6 stages. Click any badge in the job header to jump to that status at any time.</p>
        <div className="grid grid-cols-2 gap-2 mb-3">
          {[
            { s: "New", c: "#64748B", d: "Job created, not yet active" },
            { s: "Active", c: "#f26a21", d: "Work is in progress on site" },
            { s: "Drying", c: "#3B82F6", d: "Equipment placed, daily monitoring" },
            { s: "Final Inspection", c: "#e0a800", d: "Ready for final moisture check" },
            { s: "Invoicing", c: "#A855F7", d: "Scope complete, billing in progress" },
            { s: "Closed", c: "#1f9d55", d: "Job complete and paid" },
          ].map(({ s, c, d }) => (
            <div key={s} className="flex items-start gap-2 bg-[#0f1b2d] rounded-xl p-3">
        ...
        <H3>Delete a job</H3>
        <Bullet>From the <strong className="text-white">Jobs list</strong>: hover a row → trash icon appears on the right.</Bullet>
        <Bullet>From the <strong className="text-white">Job detail</strong>: trash icon in the top-right of the header → confirm delete.</Bullet>
  ...
  },
  {
    ...
    content: (
      <div>
        ...
        <H3>Add a room</H3>
        <Step n={1}>Open a job and go to the <strong className="text-white">Overview</strong> tab.</Step>
        <Step n={2}>In the <strong className="text-white">Rooms</strong> card, click <strong className="text-white">+ Add Room</strong>.</Step>
        <Step n={3}>Enter the room name (e.g. "Master Bedroom", "Kitchen"), choose the floor level, and mark whether it's affected.</Step>
        <Step n={4}>Click <strong className="text-white">Add Room</strong>. It appears immediately in the list and in all room dropdowns.</Step>
        ...
        <p className="text-sm text-slate-400">
          Rooms marked <strong className="text-white">Affected</strong> show a red dot in the overview. Non-affected rooms (e.g. rooms used for equipment staging) show a green dot.
        </p>
      </div>
    ),
  },
  {
    ...
    content: (
      <div>
        ...
        <H3>Upload photos</H3>
        <Step n={1}>Open a job → <strong className="text-white">Photos</strong> tab.</Step>
        <Step n={2}>Select a <strong className="text-white">category</strong> from the dropdown (Before / During / After / Moisture Map / Equipment / General).</Step>
        <Step n={3}>Click <strong className="text-white">Upload Photos</strong> — select one or multiple files at once.</Step>
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          {[
            ...
            <div key={c} className="bg-[#0f1b2d] rounded-xl p-3">
        ...
        <Bullet>In the full-screen view → <strong className="text-white">Delete Photo</strong> button in the top-left corner.</Bullet>
  ...
  },
  {
    ...
    content: (
      <div>
        ...
        <H3>Log a reading</H3>
        <Step n={1}>Open a job → <strong className="text-white">Moisture</strong> tab → <strong className="text-white">Add Reading</strong>.</Step>
        <Step n={2}>Select the <strong className="text-white">room</strong>, enter the <strong className="text-white">date</strong>, and describe the <strong className="text-white">location</strong> (e.g. "North wall, 12 inches up").</Step>
        <Step n={3}>Choose the <strong className="text-white">material type</strong> — this determines the dry standard.</Step>
        <Step n={4}>Enter the <strong className="text-white">moisture %</strong> from your meter and click <strong className="text-white">Save Reading</strong>.</Step>
        ...
        <div className="space-y-2 mb-3">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs text-slate-400">At or below the IICRC dry standard for that material</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-yellow-500/20 text-yellow-400">Monitoring</span>
            <p className="text-xs text-slate-400">Elevated but trending down — continue drying</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-red-500/20 text-red-400">Wet</span>
            <p className="text-xs text-slate-400">Still actively wet — equipment must remain</p>
          </div>
        </div>
  ...
  },
  {
    ...
    content: (
      <div>
        ...
        <Step n={1}>Open a job → <strong className="text-white">Equipment</strong> tab → <strong className="text-white">Log Equipment</strong>.</Step>
        <Step n={2}>Select the <strong className="text-white">equipment type</strong> and enter the <strong className="text-white">name</strong> (e.g. "Dri-Eaz LGR 7000i").</Step>
        <Step n={3}>Add the <strong className="text-white">asset number</strong> from your inventory tag (optional but recommended for billing).</Step>
        <Step n={4}>Select the <strong className="text-white">room</strong> and confirm the <strong className="text-white">date placed</strong>.</Step>

        <H3>Equipment types</H3>
        <div className="grid grid-cols-2 gap-2 mb-3">
          {["LGR Dehumidifier","Refrigerant Dehumidifier","Air Mover","HEPA Air Scrubber","HEPA Vacuum","Axial Fan","Other"].map((t) => (
            <p key={t} className="text-xs text-slate-400 bg-[#0f1b2d] rounded-lg px-3 py-2">{t}</p>
          ))}
        </div>

        <H3>Remove equipment</H3>
        <Bullet><strong className="text-white">Remove</strong> button — stamps today's date as the removal date. Equipment stays in the log for billing. Days on site is calculated automatically.</Bullet>
        <Bullet><strong className="text-white">Trash icon</strong> (hover row) — completely deletes the log entry. Use only for mistakes.</Bullet>
  ...
  },
  {
    ...
    content: (
      <div>
        ...
        <Step n={1}>Open a job → <strong className="text-white">Scope</strong> tab → <strong className="text-white">Add Line Item</strong>.</Step>
        <Step n={2}>Choose a <strong className="text-white">category</strong> (Demo / Dry / Equip / Labor / Material / Disposal / Other).</Step>
        <Step n={3}>Write a clear <strong className="text-white">description</strong> (e.g. "Remove and dispose of wet carpet — Living Room").</Step>
        <Step n={4}>Select the <strong className="text-white">room</strong> (optional), enter <strong className="text-white">quantity</strong>, <strong className="text-white">unit</strong>, and <strong className="text-white">unit price</strong>.</Step>
        <Step n={5}>The line total previews as you type. Click <strong className="text-white">Add Item</strong>.</Step>
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          {[
            ...
            <div key={u} className="bg-[#0f1b2d] rounded-xl p-2">
              <p className="text-xs font-bold text-[#f26a21]">{u}</p>
              <p className="text-xs text-slate-500">{d}</p>
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="space-y-3 mb-4">
          {[
            ...
            <div key={title} className="bg-[#0f1b2d] rounded-2xl p-4">
              <p className="text-sm font-bold text-white mb-1">{title}</p>
              <p className="text-xs text-slate-400 mb-2">{desc}</p>
              <p className="text-xs text-[#f26a21]">Requires: {needs}</p>
            </div>
          ))}
        </div>

        <H3>Generate a PDF</H3>
        <Step n={1}>Open a job → <strong className="text-white">Reports</strong> tab.</Step>
        <Step n={2}>Click <strong className="text-white">Generate PDF</strong> on the report you want.</Step>
  ...
  },
  {
    ...
    content: (
      <div>
        <p className="text-sm text-slate-400 mb-4">
          The app integrates with <strong className="text-white">Magicplan</strong> to link floor plans to jobs. Magicplan is a separate mobile app used to scan rooms and generate floor plan drawings.
        </p>

        <H3>Link an existing Magicplan project</H3>
        <Step n={1}>Open a job → <strong className="text-white">Floor Plan</strong> tab.</Step>
        <Step n={2}>Click the input field and paste your Magicplan project ID.</Step>
        <Step n={3}>Click <strong className="text-white">Save</strong>. The project is now linked.</Step>

        <H3>Create a new Magicplan project from this app</H3>
        <Step n={1}>Open the Floor Plan tab on a job with no linked project.</Step>
        <Step n={2}>Click <strong className="text-white">Create in Magicplan</strong>.</Step>
        <Step n={3}>A new project is created in your Magicplan account using the job address and details.</Step>

        <H3>Sync floor plans</H3>
        <Step n={1}>Once a project is linked, click <strong className="text-white">Sync Now</strong> to pull the latest exported floor plan.</Step>
  ...
  },
  {
    ...
    content: (
      <div>
        <p className="text-sm text-slate-400 mb-4">
          Manage who has access to the app from the <strong className="text-white">Settings</strong> page. Only administrators can invite users or change roles.
        </p>

        <H3>Invite a new user</H3>
        <Step n={1}>Go to <strong className="text-white">Settings</strong> → <strong className="text-white">Invite User</strong>.</Step>
        <Step n={2}>Enter their name and email address.</Step>
        <Step n={3}>Select their role (see below).</Step>
        <Step n={4}>Click <strong className="text-white">Send Magic Link</strong>. They receive an email with a one-click login link — no password needed.</Step>

        <H3>Roles</H3>
        <div className="space-y-2 mb-4">
          {[
            { role: "Admin", c: "#f26a21", d: "Full access — create/edit/delete jobs, manage users, access all settings. Typically for office staff and project managers." },
            ...
            <div key={role} className="bg-[#0f1b2d] rounded-xl p-3">
        ...
        <Bullet>Go to <strong className="text-white">Settings</strong> → find the user → click their current role badge → select a new role.</Bullet>

        <H3>Access on iPhone</H3>
        <p className="text-sm text-slate-400">Open <strong className="text-white">Safari</strong> on your iPhone and navigate to the app URL. If you're on the same WiFi as the office computer, use the local network address. For permanent access from anywhere, the app can be deployed to a public URL — contact your administrator.</p>
      </div>
    ),
  },
];
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-xl bg-[#f26a21]/15 border border-[#f26a21]/30 flex items-center justify-center">
            <BookOpen size={20} className="text-[#f26a21]" />
          </div>
          <div>
            <h1 className="text-2xl font-black text-white">Help & User Guide</h1>
            <p className="text-sm text-slate-500">Roybal Construction LLC — Field Operations App</p>
          </div>
        </div>
      ...
      {/* Quick reference */}
      <div className="bg-[#0f1b2d] border border-[#1f3354] rounded-2xl p-5 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400">
          {[
            ...
            <span key={i} className="flex items-center gap-1.5">
              <span className="bg-[#f26a21]/15 text-[#f26a21] px-2.5 py-1 rounded-lg font-medium">{step}</span>
      ...
      <div className="space-y-2">
        {SECTIONS.map((section) => {
          ...
          return (
            <div
              key={section.id}
              className="bg-[#0f1b2d] border border-[#1f3354] rounded-2xl overflow-hidden"
            >
              <button
                onClick={() => toggle(section.id)}
                className="w-full flex items-center gap-3 px-5 py-4 text-left hover:bg-[#1f3354]/30 transition-colors"
              >
                <span
              ...
              {isOpen && (
                <div className="px-5 pb-5 border-t border-[#1f3354]">
      ...
      <div className="mt-8 text-center">
        <p className="text-xs text-slate-600">Roybal Construction LLC — Field Operations App</p>
        <p className="text-xs text-slate-700 mt-1">For technical support, contact your administrator.</p>
      </div>
    </div>
  );
}
