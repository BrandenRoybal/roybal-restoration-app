    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-[#f26a21] hover:bg-[#d4520f] text-[#16263d] font-bold px-4 h-10 rounded-xl transition-colors"
        >
          <Plus size={18} />
          New Job
        </button>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full bg-[#0f1b2d] border border-[#1f3354] rounded-xl pl-9 pr-9 h-10 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-[#f26a21] transition-colors"
          />
          {search && (
          ...
          {(["all", ...JOB_STATUS_ORDER] as const).map((s) => (
            <button
              ...
              className={clsx(
                ...
                statusFilter === s
                  ? "bg-[#f26a21] border-[#f26a21] text-[#16263d]"
                  : "bg-[#0f1b2d] border-[#1f3354] text-slate-400 hover:text-slate-200"
              )}
            >
              ...
              {loading ? (
                ...
                filtered.map((job) => (
                  confirmDeleteId === job.id ? (
                    <tr key={job.id} className="border-b border-[#1f3354]/50 bg-red-950/20">
                      <td colSpan={8} className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            className="px-3 h-8 bg-[#1f3354] text-slate-300 text-xs font-bold rounded-lg hover:bg-[#4A4440] transition-colors"
                          >
                            Cancel
                          </button>
                        </div>
                    ...
                    <tr
                      key={job.id}
                      className="border-b border-[#1f3354]/50 hover:bg-[#16263d] cursor-pointer transition-colors group"
                    >
                      <td className="px-4 py-3" onClick={() => navigate(`/jobs/${job.id}`)}>
                        <span className="font-mono text-xs text-slate-400 group-hover:text-[#f26a21] transition-colors">
                          {job.job_number}
                        </span>
                      </td>
